...
# Trea Behaviour Tree 使用说明（USE.md）
...
Trea Behaviour Tree（包名 `com.trea.behaviortree`，当前版本 `0.1.0`）是一套**新手友好、轻量、无外部依赖**的 Unity 行为树 AI 插件，命名空间统一为 `Trea`。它提供：
...
| 显示名（`displayName`） | Trea Behaviour Tree |
...
`Samples~` 目录遵循 UPM 规范，**不会**被自动编译或打入包。在 Package Manager 中选中 `Trea Behaviour Tree` → `Samples` → `Import` 导入「示例敌人 AI」后，即可在项目中出现 `TreaExample` 示例（含 `Trea/创建示例敌人` 菜单）。
