...
MengShenBehaviourTree（包名 `com.trea.behaviortree`，当前版本 `0.1.0`）是一套轻量、无外部依赖的 Unity 行为树 AI 插件，命名空间统一为 `Trea`。仓库根目录即 UPM 包，可直接通过 Package Manager 安装。
...
- 每个资产对应一个节点：在 Inspector 中设置 **节点类型**（`ActionType` 枚举，见下文「节点详解」）、**节点名称**与对应参数；
...
- 菜单 `Trea/示例/创建示例节点资产` 可一键在 `Assets/TreaExample` 下生成一套示例资产链。
...
- 运行时可在 Inspector 中观察节点状态颜色（见下文「编辑器工具」）；
...
`ActionNode` / `ConditionNode` 封装了更友好的接口（见下文「自定义行为节点」）。

### 组合节点（CompositeNode，多个子节点）
...
> Tick 返回值语义：`Running` = 未完成，下个 tick 继续；`Success` / `Failure` = 本次执行结束。

### 资产方式下可选的节点类型（ActionType 枚举）
...
Wait, Log, Chance, Animation, MoveTo, Custom           // 叶（Custom 见「自定义行为节点」）
```

对应资产参数（Inspector 按类型显示）：
...
- 运行时可在 Runner Inspector 的「黑板数据」区域查看所有键值。
...
### BehaviourTreeRunnerEditor（Runner Inspector）
...
### TreaAction 资产 Inspector（TreaActionEditors）
...
Package Manager 里选中本包 → Samples → Import，导入「示例敌人 AI」；或使用菜单 `Trea/创建示例敌人` 一键生成（需给玩家物体加 `Health` 组件并设置 Tag 为 `Player`）。

`ExampleEnemy` 演示了完整用法。构建出的树结构（`ExampleEnemyAI.BuildTree()`）：
